...
export default function Slide(props: SlideProps) {
  ...
  const [animateKeyFrame, setAnimateKeyFrame] = useState(isPDFExport() ? props.animateKeyFrames ?? 0 : 0);
  ...
  const visible = slideIndex !== undefined && slideIndex === presentationState?.currentSlideIndex;
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleDirection = (ev: Event, direction: "next" | "prev" | "none") => {
      if (direction === "next") {
  ...
  }, [presentationState?.reveal, animateKeyFrames, animateKeyFrame, visible, slideIndex, presentationState]);
